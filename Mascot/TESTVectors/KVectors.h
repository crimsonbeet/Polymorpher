...
struct KWinPoint { 
	...
	inline void GDIPoint(POINT& pt/*out*/, double ratio = 1.0) const { 
		pt.x = (int)(_x * ratio + 0.45); 
		pt.y = (int)(_y * ratio + 0.45); 
	} 
}; 
	...
	void Draw(const KWinDrawParams& params) { 
		...
		while(iter != _meta_map.end()) { 
			...
			if(obj->brush.lbStyle != 0 || obj->brush.lbHatch != 0 || obj->brush.lbColor !=  RGB(0, 0, 0)) { 
				new_pen = ExtCreatePen(PS_GEOMETRIC | PS_SOLID, params._pen_width > 1? (int)(params._pen_width + 0.5): 1, &obj->brush, 0, NULL); 
...
struct XXX { 
	int int_arr[21]; 
	std::string str_arr[41]; 
	double dbl; 
}; 

BEGIN_WSI_SERIALIZATION_OBJECT(XXX) 

CONTAINS_FLAT_MEMBER(dbl, D) 
CONTAINS_FLAT_MEMBER(int_arr, I) 
CONTAINS_FLAT_MEMBER(str_arr, S) 
END_WSI_SERIALIZATION_OBJECT() 
