...
template<typename T> 
#if _MSC_VER > 1700
struct thread_objects_map: public std::unordered_map<DWORD, T>, public wsi_gate { 
#else 
...
public:
	...
	int _writer_type; // 'W' - wsi, 'X' - xml, 'J' - json, 'F' - fep, 'D' - dart, 'H' - http, 'R' - raw string, 'V' - CRF vision data.
	int _lazy_start; 
...
public:
	...
	XSubscriber _request; // The other end is a subscriber to the Connector owning this XProducer.
	int _status;
	int _freeze;
	...
	bool IsKeepAlive() {
		...
		_timeb checktime;
		_ftime(&checktime);
		if(_request._writer.IsValid() && (_lastcheckpoint.time + 30) < checktime.time) {
			return true; 
		}
#endif
		return false; 
...
public:
#if _MSC_VER > 1700
	class subscribers_map: public std::unordered_map<std::string, XSubscriber*>, public wsi_gate {
#else 
...
//		key field has format 999.999.999.999:NAME
...
#if _MSC_VER > 1700
	std::unordered_map<std::string, XSubscriber*>::iterator FindMappedSubscriber(const std::string& key);
	std::unordered_map<std::string, XSubscriber*>::iterator FindMappedSubscriber(const SOCKET& sock);
#else
	...
	bool ShutdownSubscribers(const std::string& addr, XSubscriber *possible_clone = 0);

public:
#if _MSC_VER > 1700
	class producers_map: public std::unordered_map<std::string, XProducer*>, public wsi_gate {
#else
