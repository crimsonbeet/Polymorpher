...
size_t recvn(SOCKET sock, char *vptr, size_t nsize, int flags, int timeout_secs, bool one_step) {
	...
	while(nleft > 0) {
		...
		if(navailable == 0 && timeout_secs >= 0) {
			...
			FD_ZERO(&ssock);
			FD_SET(sock, &ssock);
			if(timeout_secs == 0 || select(sock + 1, &ssock, NULL, NULL, &tv) <= 0) {
				if(_g_trace & 8) {
					if(timeout_secs != 0) { 
		...
		while((int)(nread = recv(sock, ptr, nleft, flags)) < 0) {
			if(timeout_secs < 0) { 
	...
	if(is_fep) {
		...
		if(length <= nread) {
			...
			if(nread) {
				...
				std::string next_msg;
				if(read_fepmessage(sock, next_header, next_msg, next_header.size())) {
			...
			memcpy((PCHAR)msg.data(), (PCHAR)header.data(), nread);
			length -= nread;
			nread = recvn(sock, (PCHAR)msg.data() + nread, length, 0, 1/*secs time-out*/);
			if(nread != length) {
				is_fep = FALSE;
			}
		}
...
//	read argument buffer from client.
	msg.resize(0);
	int nread = recvn(sock, (PCHAR)&header, 4/*bytes*/, 0, timeout_secs);
	...
	if((nread += recvn(sock, ((PCHAR)&header) + 4, sizeof(WSIHeader) - 4, 0, 0/*secs time-out*/)) != (int)sizeof(WSIHeader)) {
		if(_g_trace) {
...
//		04/27/2009 - Send version object on connect.
	...
	if(header._aux == IPCObj_ReceiveObj || header._aux == IPCObj_FepMessage || header._aux == IPCObj_DartMessage || header._aux == IPCObj_HttpMessage || header._aux == IPCObj_RawData) {
