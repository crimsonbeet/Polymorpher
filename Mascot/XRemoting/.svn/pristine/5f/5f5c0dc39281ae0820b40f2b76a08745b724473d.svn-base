...
void CWsiSerializerBase::reset_all_fundamental_types(_w64 int outer_offset) {
	LateBinding();
...
void CWsiSerializerBase::current_binding_erase_all(_w64 int outer_offset) {
	LateBinding();
	converter_vector::iterator iter = _pvector->begin();
	while(iter != _pvector->end()) {
		if((*iter)->_serializer != this) {
...
void CWsiSerializerBase::operator()(ISTREAMWriter& writer, const void *obj, std::string& wsi) /*returns string image*/ {
	_outer_offset = (int)((char*)obj - (char*)_inner_ptr);
...
void CWsiSerializerBase::operator()(ISTREAMWriter& writer, std::string& wsi) /*returns string image*/ {
	...
	reset_all_fundamental_types(_outer_offset);
	get_string_image(0/*idx*/, writer.NoEmptyRootObjects()/*can_be_empty=false*/);
	...
	switch(action) {
		...
		case 3: /*erase*/
			for(_next = 0; _next < _last; ++_next) {
				...
				This->current_binding_erase_all(This->_outer_offset + (int)((char*)This->_inner_ptr - (char*)GetElement(0)));
			}
			_next = 0;
			break;
	}
...
bool IWsiVectorSerializer::MyCallFunctor(int action, CWsiSerializerBase *This) {
	...
	switch(action) {
		case 0: /*reset*/
			...
			if(Size() > 0) {
				_next = 1;
				This->reset_all_fundamental_types(_addrint(This->set_inner_ptr(GetElement(0))));
			}
			else {
				_next = 0;
		...
		case 2: /*get*/
			if(_next <= Size() && This->get_inner_ptr()) {
				This->get_string_image(_next, !This->_writer_ptr->PutAll(true));
				if(_next < Size()) {
					This->reset_all_fundamental_types(_addrint(This->set_inner_ptr(GetElement(_next))));
				}
				++_next; 
				rc = true;
			}
	...
	if(_next < _last) {
		if(Size() < (++_next)) {
			...
			while(last_check_point < _next) {
				This->current_binding_erase_all(_addrint(This->set_inner_ptr(GetElement(last_check_point++))));
			}
		}
		else {
			This->reset_all_fundamental_types(_addrint(This->set_inner_ptr(GetElement(_next - 1))));
		}
		rc = true;
	}
