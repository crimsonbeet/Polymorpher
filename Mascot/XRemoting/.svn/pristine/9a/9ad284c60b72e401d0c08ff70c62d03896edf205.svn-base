...
bool SSocketCtl::Close(SSocketCtl& ctl, SSocketCtl *possible_clone) {
	bool rc = false; 
	if(ctl._sock != INVALID_SOCKET) {
		ctl.LOCKER.lock();
		if(ctl._sock != INVALID_SOCKET) {
			if(possible_clone && possible_clone->_sock == ctl._sock) {
				possible_clone->_sock = INVALID_SOCKET; 
				possible_clone->Reset(); 
			}
