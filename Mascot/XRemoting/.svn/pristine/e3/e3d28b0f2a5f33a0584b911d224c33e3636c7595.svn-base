...
BOOL IPCSendMessage(const sockaddr_in *paddr, volatile SOCKET& target, IPCMsgheader& copydata, IWSIMarshaller *marsh = 0, XConnector *conn = 0) {
	...
	if(target == INVALID_SOCKET && paddr) { 
		...
		if(target != INVALID_SOCKET) { 
			DWORD in_buffer_size = (DWORD)copydata._in_buffer_size; 
			DWORD out_buffer_size = (DWORD)copydata._out_buffer_size; 
			DWORD nonblocking = 0;
			BOOL nodelay = 1; 
...
void add_client::operator() (const XSocket2Handler<XProducer>& handler) {
	...
	if((int)client_sock > (int)_maxsock) {
...
void poll_client::operator() (sockclients_map::value_type& client) {
	...
	if(sock != INVALID_SOCKET && !ok) { 
		...
		if(client.second._handler && client.second._handler->_status == IPCReq_CONFIRMED) { // 02/14/2016
			client.second._handler->OnStatusChange(IPCReq_CLOSED);
		}
		else 
		if(!_ctl._pConn->DeleteSubscriber(sock_value, true)) { // 10/20/2008 - Dynamic subscriber reuses existing connection. Try to delete it.
			if(_g_trace & 4) {
...
poll_client::~poll_client() {
	...
	while(iter != _clmap.end()) {
		if((*iter).second._sock == INVALID_SOCKET) {
			...
			if((int)_maxsock < (int)((*iter).second._sock)) {
...
return_t CALLBACK XReadingHead::Monitor(PVOID pv) {
	...
	while(ok) {
		...
		if((int)handler >= 0 && (int)maxsock < (int)handler) {
			maxsock = handler;
		}
		...
		int n = 0;
		if((int)maxsock >= 0) {
			n = select((int)(maxsock + 1), &rset, NULL, NULL, &tv);
		}
		else {
			OSSleep(200);
		}
...
int IPCRequestType(XWritingHead *writing_head) {
	...
	case (int)'V': rt = IPCObj_CRFData; break;
...
bool XWritingHead::Connect(std::string* my_addr_out) {
	...
	if(!IsValid() && _host_ptr->_addr.size()) {
		...
		if(_host_ptr->_static) {
			...
			if(!rc && copydata._tag) {
				_host_ptr->FireException(3, copydata._tag, FALSE);
			}
		}
	}
...
void XWritingHead::KeepAlive() {
	if(!IsLocalAddress() && IsValid() && _host_ptr->_addr.size()) {
		IPCMsgheader copydata(Connector()->_name, IsDefaultWriter() ? IPCObj_Connect : IPCObj_InvalidHeader, (char)_host_ptr->_writer_type);

		copydata._ticketexpected = '0';

		IPCSendMessage(NULL, _target._sock, copydata, NULL, NULL);
	}
}
