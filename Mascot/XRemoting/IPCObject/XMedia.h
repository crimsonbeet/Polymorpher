...
struct SSocketCtl: CRefCounter {
	...
	void Reset(); 
	static bool Close(SSocketCtl& ctl, SSocketCtl *possible_clone = 0);
};
...
#if _MSC_VER > 1700
typedef std::unordered_map<SOCKET, XSocket2Handler<XProducer> > sockclients_map;
#else
...
public:
	...
	bool Shutdown(XWritingHead *possible_clone = 0);
