...
bool XConnector::ShutdownSubscribers(const std::string& addr, XSubscriber *possible_clone) {
	...
	for(size_t x = 0; x < subscribers.size(); ++x) {
		subscribers[x]->_writer.Shutdown(possible_clone? &possible_clone->_writer: 0);
...
//				Shutdown a possible static subscriber that points to same server.
				_host_ptr->ShutdownSubscribers(_request._addr, &_request);
...
return_t CALLBACK Thread_SendRequest(PVOID pv) {
	if(pv == NULL) {
		return 0;
	}
	XProducer *producer = (XProducer*)pv;
	producer->SendRequest(1);
	InterlockedDecrement(&producer->_host_ptr->_activerequests_count);
	return 0;
}
...
void XProducer::OnCheck(_timeb& checktime, std::vector<XSocket2Handler<XProducer> * >& new_clients) {
	...
	if(_status == IPCReq_NOTPROCESSED) {
		if(!_freeze) {
			this->_status = IPCReq_WAITING; 
			InterlockedIncrement(&_host_ptr->_activerequests_count);
			QueueWorkItem(Thread_SendRequest, (PVOID)this);
		}
	}
	if(_status == IPCReq_WAITING) {
		return;
	}
