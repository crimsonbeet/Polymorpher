...
It concatenates those streamified arrays together, so the output array containes a compressed image of the input array. 
...
void compress_enhanced_graphics(std::vector<short int>& short_data, std::string& vStreamed_values) { 
	...
	for(size_t i = 0; i < 3; ++i) { 
		...
		for(size_t z = nRSize >> 1; z < vTransformed_values.size(); ++z) { 
...
void decompress_enhanced_graphics(std::string& vStreamed_values, std::vector<short int>& short_data) { 
	...
	for(size_t i = 0; i < 3; ++i) { 
		...
		memcpy(vValues.data(), &vStreamed_values[offset + 2], length); 
