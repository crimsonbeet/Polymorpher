...
#if _MSC_VER > 1700
	typedef std::unordered_map<substring_locator, converter_tuple, std::hash<std::string>> converter_map;
#else 
...
//	resets iterators of current binding.
	void reset_all_fundamental_types(_w64 int outer_offset);

//	erases all memebrs of current binding.
	void current_binding_erase_all(_w64 int outer_offset);
...
//	It contains a pointer to currrent instance of a base class, a member of which this class represents.
	...
	void SetMemBase(_w64 int p_mem) {
		_mem_base = GetBase() + _addrint(p_mem);
	}
...
#if _MSC_VER > 1700
std::unordered_map<std::string, IWsiSerializerBase* > CWsiRootCollection::_map;
#else
