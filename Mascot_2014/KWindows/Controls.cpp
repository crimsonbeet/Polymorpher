...
BOOL EXP_KWIN KWinCreateImageList(const SCreateImageList& params, SImageList& iml) { 
	...
	iml._himl = ImageList_Create ( 
		...
		params._images.size(),	// Number of images that the image list initially contains 
...
//		RBBIM_TEXT |				// lpText is valid
	...
	int band_count = SendMessage(params._hwndrebar, RB_GETBANDCOUNT, 0, 0); 
...
LRESULT CALLBACK KWinComboWndProc(HWND hWnd, UINT uMessage, WPARAM wParam, LPARAM lParam) {
	if((uMessage == WM_COMMAND) && (HIWORD(wParam) == CBN_SELCHANGE))	{ 
		int iselect = SendMessage(hWnd, CB_GETCURSEL, 0, 0); 
		if(iselect != CB_ERR) { 
			PSComboBox pcombo = (PSComboBox)(LONG_PTR)GetWindowLongPtr(hWnd, GWLP_USERDATA); 
			if(pcombo) { 
				int ilength = SendMessage(hWnd, CB_GETLBTEXTLEN, (WPARAM)iselect, 0); 
				if(ilength != CB_ERR) { 
...
BOOL EXP_KWIN KWinCreateComboBox(SCreateComboBox& params, std::auto_ptr<SComboBox> *ppcombo) { 
	...
	while(iter != params._strings.end()) { 
		int idxString = iter - params._strings.begin(); 
		int idxImage = -1; 
		if(params._piml) { 
			if(params._piml->_indices.size() > (size_t)idxString) { 
				idxImage = params._piml->_indices[idxString]; 
			} 
		} 
...
//		bt._state = 0; 
	...
	while(iter != toolbar._buttons.end()) { 
		...
		button.iBitmap = (iter - toolbar._buttons.begin()); 
...
//		TBSTYLE_TRANSPARENT | 
		...
		0, 
		0, 
		isize * numbitmaps/*CW_USEDEFAULT*/, 
		isize, 
		params._hwndparent, 
