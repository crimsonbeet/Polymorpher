...
LRESULT CALLBACK KHyperlist::ListBoxSubclassWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) { 
	...
	KHyperlist *pWnd = 0; 
	if(hParent) { 
		pWnd = (KHyperlist *)(LONG_PTR)GetWindowLong(hParent, GWL_USERDATA); 
//		pWnd = (KHyperlist *)GetWindowLongPtr(hParent, 0); 
	} 
...
LRESULT CALLBACK KHyperlist::EditBoxSubclassWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) { 
	...
	KHyperlist *pWnd = 0; 
	if(hMainParent) { 
		pWnd = (KHyperlist *)(LONG_PTR)GetWindowLong(hMainParent, GWL_USERDATA); 
	} 
