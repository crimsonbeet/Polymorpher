...
/*
 * This template gets a string from the first item of a vector. 
 * The type should be either a string, or a class implementing operator std::string().
 */
template <typename T> 
std::string& operator += (std::string& str, std::vector<T>& v) { 
	if(v.size()) { 
		return str += (std::string)v[0]; 
	} 
	return str; 
} 
