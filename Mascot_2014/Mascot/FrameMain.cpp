...
//			cr_rebar._piml = &_iml; 
		...
		if(_veccanvas.size() > 1) { 
			_g_drawing_frame = new XFrameDrawing(_hinst, _veccanvas[1]._hwnd); 
			_g_drawing_frame->SetTopMost(); 

			_g_frame_history->InsertHistory(_g_drawing_frame); 

			IPCSetLogHandler(_g_drawing_frame->_hwnd); 
		} 
		nstep = 0; 
		break; 
	} 
	...
	switch(params._umsg) { 
		...
		if(IsSizing()) { 
			...
			if(_veccanvas.size() > 1) { 
				...
				MoveWindow(_g_drawing_frame->_hwnd, 0, 0, clrect.right, clrect.bottom, TRUE); 
			} 
		} 
		break; 
	} 
